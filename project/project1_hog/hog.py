...
def swine_align(player_score, opponent_score):
    ...
    def gcd(x, y):
        ...
        while k > 1:
            ...
            k -= 1
        return k
    if gcd(opponent_score, player_score) >= 10:
        return True
    return False
    # END PROBLEM 4a
...
def pig_pass(player_score, opponent_score):
    ...
    if player_score >= opponent_score:
        return False
    score_differ = opponent_score - player_score
    if score_differ < 3:
        return True
    return False
    # END PROBLEM 4b
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if score - last_score > running_high:
            run_h = score - last_score
            print(run_h, 'point(s)! The most yet for Player', who)

        return announce_highest(who, score, run_h)
    return say
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    highest_scores = 0
    while roll_num <= 10:
        average_result = make_averaged(roll_dice, trials_count)(roll_num, dice)

        if average_result > highest_scores:
            highest_scores = average_result
            num = roll_num

        elif average_result == highest_scores:
            if roll_num < num:
                num = roll_num
        roll_num += 1
    return num
...
def bacon_strategy(scores, opponent_score, cutoff=8, num_rolls=6):
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if not bacon_strategy(score, opponent_score, cutoff, num_rolls) or swine_align(score_new, opponent_score) or pig_pass(score_new, opponent_score):
        return 0
    else:
        return num_rolls
